...
class Spider:
    def __init__(self):
        ...
        self.tool=tool.Tool();#引入工具类  正则表达式
    ...
    def savePagesInfo(self,pageIndex):
        ...
        pattern = re.compile(
            '<div class="list-item".*?pic-word.*?<a href="(.*?)".*?<img src="(.*?)".*?<a class="lady-name.*?>(.*?)</a>.*?<strong>(.*?)<>.*?<span>(.*?)</span>',
            re.S);#引入正则表达式
        ...
        for item in contents: 
            ...
            albumRsponse=urllib.request.urlopen(albumsUrl);
            albumRsponse=albumReponse.read().decode("gbk");
            #print(albumRsponse);
            ...
            for item in mmAlbums:
                ...
                pageNumber=1;
                mm_link="https://mm.taobao.com/album/json/get_album_photo_list.htm?user_id="+params["user_id"][0]+"&album_id="+parame["album_id"][0]+"&top_pic_id=0&cover=%2F%2Fimg.alicdn.com%2Fimgextra%2Fi1%2F687471686%2FTB1jlwCHXXXXXcvXXXXXXXXXXXX_!!687471686-0-tstar.jpg&page="+str(pageNumber)+"&_ksTS=1502681447738_154&callback=jsonp155"
                ...
                for index,oneImage in enumerate(imagesList):
                    if (os.path.exists(albumPath+"/"+str(index)+".jpg")):
                        print(albumPath+"/"+str(index)+".jpg已经存在了");
                        continue;
