    ...
    for x in iter_x:
        ...
        if cate:
            ...
            for i, v in enumerate(cate_name_1):
                if v in cate_name_2 and v in cate_name_3 and cate_name_2.index(v) == i and cate_name_3.index(v) == i:
                    res.append(v)
                else:
                    res.append(' ')
        ...
        for i, v in enumerate(_):
            if u'地址' in v and len(_) > i + 1:
                com_addr = _[i + 1].split('\n')[0]
            if u'电话' in v and len(_) > i + 1:
                telephone = _[i + 1]
        if not com_name:
        ...
        for i in doc('div.addres p').items():
            if u'地址' in i.text() and not com_addr:
                com_addr = i.text().split(u'：')[-1]
            if u'电话' in i.text() and not telephone:
        ...
        for i in doc('div.basic-info table td').items():
            if i('span.first').text() == u'在线联系':
                # try:
                #     params = i('span.first').text().split('?')[-1].split('&')
                #     base_url = i('span.first').text().split('?')[0]
                # except:
                res = []
                try:
        ...
        try:
            db['hy88'].update_one({"_id": source_url, }, {"$set": _}, upsert=True)
        except:
            try:
                mongo_client = pymongo.MongoClient("192.168.14.90", 27017)
                db = mongo_client["final_field"]
